...
function goTo(page, opts = {}) {
  ...
  else if (!opts.replace) State.navStack.push(page);
  ...
  if (navEl) navEl.classList.toggle('hidden', fullscreen.includes(page));
  refreshIcons();
  ...
  // PWA: pushState للتعامل مع زر الرجوع في الجوال
  // (لا نستخدم URL حقيقي لأن التطبيق single-page)
  if (!opts.fromPopState && !opts.replace) {
    try { history.pushState({ page, ts: Date.now() }, '', null); } catch(e) {}
  }
}
...
// زر الرجوع في الجوال (Android/iOS) — يرجع خطوة بدل الخروج من التطبيق
window.addEventListener('popstate', (e) => {
  if (State.navStack.length > 1) {
    State.navStack.pop();
    const prev = State.navStack[State.navStack.length - 1];
    goTo(prev, { replace: true, fromPopState: true });
  } else {
    // إذا في الصفحة الرئيسية — نضيف entry جديد عشان ما يخرج
    try { history.pushState({ page: 'home' }, '', null); } catch(e) {}
  }
});
        ...
        ${sub.topics.map((t,ti)=>{
          ...
          return `
            <button class="topic-item" onclick="openTopic('${sub.id}',${ti},'${c.id}')">
              ...
              ${hd?`<span class="badge badge-success">${ic('check',10)}</span>`:''}
              ${ic('chevron-left',14)}
            </button>
          `;
        }).join('')}
...
function topicFull(d, key) {
  ...
  return `
    ...
    <!-- ===== تبويبات الموضوع ===== -->
    <div class="topic-tabs fade-up delay-1" id="tabs-${key}">
      <button class="topic-tab active" onclick="switchTopicTab('content','${key}',this)">${ic('book-open',13)} المحتوى</button>
      <button class="topic-tab" id="tab-videos-${key}" onclick="switchTopicTab('videos','${key}',this)" style="${(window.APP_DATA.TOPIC_MEDIA&&window.APP_DATA.TOPIC_MEDIA['${key}']&&window.APP_DATA.TOPIC_MEDIA['${key}'].videos&&window.APP_DATA.TOPIC_MEDIA['${key}'].videos.length)?'':'display:none'}">${ic('play-circle',13)} فيديوهات</button>
      <button class="topic-tab" id="tab-images-${key}" onclick="switchTopicTab('images','${key}',this)" style="${(window.APP_DATA.TOPIC_MEDIA&&window.APP_DATA.TOPIC_MEDIA['${key}']&&window.APP_DATA.TOPIC_MEDIA['${key}'].images&&window.APP_DATA.TOPIC_MEDIA['${key}'].images.length)?'':'display:none'}">${ic('image',13)} صور</button>
    </div>

    <!-- محتوى التبويب: المحتوى -->
    <div id="tabpane-content-${key}" class="topic-tabpane active-pane">
    ...
    <!-- محتوى التبويب: فيديوهات -->
    <div id="tabpane-videos-${key}" class="topic-tabpane" style="display:none">
      ${(()=>{const m=window.APP_DATA.TOPIC_MEDIA&&window.APP_DATA.TOPIC_MEDIA[key];if(!m||!m.videos||!m.videos.length)return'<div class="topic-section" style="text-align:center;padding:32px"><p style="color:var(--text-muted);font-size:12px">لا توجد فيديوهات بعد</p></div>';return`<div class="px-5" style="padding-top:12px">${m.videos.map((v,i)=>`<div class="video-card fade-up" style="margin-bottom:12px;background:var(--surface);border:1px solid var(--border);border-radius:14px;overflow:hidden"><div class="video-thumb-wrap" style="position:relative;height:0;padding-bottom:56.25%;background:linear-gradient(135deg,var(--cat-700),var(--cat-500));cursor:pointer" onclick="playYT('yt-${key}-${i}','${v.id}',this)"><div id="yt-ph-${key}-${i}" style="position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:10px"><div style="width:52px;height:52px;border-radius:50%;background:rgba(255,255,255,.15);border:2px solid rgba(255,255,255,.4);display:flex;align-items:center;justify-content:center">${ic('play',20)}</div><span style="font-size:10px;color:rgba(255,255,255,.6)">اضغط للتشغيل</span></div><iframe id="yt-${key}-${i}" style="position:absolute;inset:0;width:100%;height:100%;display:none;border:none" allowfullscreen allow="autoplay;encrypted-media"></iframe><span style="position:absolute;bottom:8px;left:8px;background:rgba(0,0,0,.75);color:white;font-size:9px;font-weight:700;padding:2px 7px;border-radius:5px;font-family:monospace">${v.duration}</span></div><div style="padding:10px 12px;display:flex;align-items:center;gap:8px"><div style="width:28px;height:28px;border-radius:7px;background:#ff0000;display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:11px;color:white">▶</div><div style="flex:1;text-align:right"><p style="font-size:11px;font-weight:700;margin:0 0 2px;color:var(--text)">${v.title}</p><p style="font-size:9px;color:var(--text-muted);margin:0">${v.subtitle}</p></div></div></div>`).join('')}</div>`})()}
    </div>

    <!-- محتوى التبويب: صور -->
    <div id="tabpane-images-${key}" class="topic-tabpane" style="display:none">
      ${(()=>{const m=window.APP_DATA.TOPIC_MEDIA&&window.APP_DATA.TOPIC_MEDIA[key];if(!m||!m.images||!m.images.length)return'<div class="topic-section" style="text-align:center;padding:32px"><p style="color:var(--text-muted);font-size:12px">لا توجد صور بعد</p></div>';return`<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;padding:12px 20px">${m.images.map((img,i)=>`<div style="background:var(--surface);border:1px solid var(--border);border-radius:12px;overflow:hidden;cursor:pointer" onclick="openImagePage('${key}',${i})"><img src="${img.url}" style="width:100%;height:90px;object-fit:cover;display:block" onerror="this.style.display='none'" loading="lazy"><div style="padding:8px 10px;font-size:10px;font-weight:600;color:var(--text-muted);text-align:right">${img.title}</div></div>`).join('')}</div>`})()}
    </div>
...
function init() {
  ...
  // أول pushState entry — عشان زر الرجوع في الجوال يشتغل من البداية
  try { history.replaceState({ page: 'home', ts: Date.now() }, '', null); } catch(e) {}
}

// ===== دوال التبويبات والوسائط =====
function switchTopicTab(tab, key, btn) {
  // إخفاء كل tabpanes
  ['content','videos','images'].forEach(t => {
    const el = document.getElementById('tabpane-' + t + '-' + key);
    if (el) el.style.display = 'none';
  });
  // إخفاء كل أزرار التبويب
  const tabsEl = document.getElementById('tabs-' + key);
  if (tabsEl) tabsEl.querySelectorAll('.topic-tab').forEach(b => b.classList.remove('active'));
  // إظهار المحدد
  const target = document.getElementById('tabpane-' + tab + '-' + key);
  if (target) target.style.display = 'block';
  if (btn) btn.classList.add('active');
}

function playYT(iframeId, videoId, container) {
  const iframe = document.getElementById(iframeId);
  const phId = iframeId.replace('yt-', 'yt-ph-');
...
function openImagePage(key, idx) {
  const m = window.APP_DATA.TOPIC_MEDIA && window.APP_DATA.TOPIC_MEDIA[key];
  if (!m || !m.images || !m.images[idx]) return;
  const img = m.images[idx];
  State.imagePageData = { key, idx, img };
  goTo('image-detail');
}
...
function renderImageDetail() {
  ...
  $('#page-image-detail').innerHTML = `
    <div class="top-bar">
      <button class="icon-btn" onclick="goBack()">${ic('arrow-right')}</button>
      <div class="top-bar-title"><span style="font-size:13px;font-weight:700">${img.title}</span></div>
      <div style="width:40px"></div>
    </div>
    <div style="overflow-y:auto;height:calc(100% - 56px)">
      <div style="padding:16px 20px;background:var(--surface);border-bottom:1px solid var(--border)">
        <p style="font-size:13px;line-height:1.8;color:var(--text)">${img.description || ''}</p>
      </div>
      <div style="padding:16px 20px;cursor:pointer;text-align:center" onclick="openLightbox('${img.url}','${img.title}')">
        <img src="${img.url}" style="width:100%;border-radius:12px;border:1px solid var(--border)" onerror="this.parentElement.innerHTML='<p style=color:var(--text-muted)>الصورة غير متاحة</p>'" loading="lazy">
        <p style="font-size:10px;color:var(--text-muted);margin-top:8px">${ic('zoom-in',12)} اضغط لتكبير الصورة</p>
      </div>
      ${m && m.images.length > 1 ? `<div style="display:flex;gap:8px;padding:0 20px 20px;overflow-x:auto">${m.images.map((im,i) => `<div style="flex-shrink:0;width:70px;height:60px;border-radius:8px;overflow:hidden;cursor:pointer;border:2px solid ${i===idx?'var(--cat-500)':'var(--border)'}" onclick="openImagePage('${key}',${i})"><img src="${im.url}" style="width:100%;height:100%;object-fit:cover" onerror="this.style.display='none'"></div>`).join('')}</div>` : ''}
    </div>`;
}
...
function openLightbox(url, title) {
  ...
  if (!lb) {
    ...
    lb.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,.92);z-index:99999;display:flex;align-items:center;justify-content:center;flex-direction:column;gap:12px;padding:16px';
    lb.onclick = () => lb.remove();
    document.body.appendChild(lb);
  }
  lb.innerHTML = `<img src="${url}" style="max-width:100%;max-height:80vh;border-radius:12px;object-fit:contain"><p style="color:rgba(255,255,255,.6);font-size:11px">${title}</p><button onclick="document.getElementById('global-lightbox').remove()" style="background:rgba(255,255,255,.1);border:1px solid rgba(255,255,255,.2);color:white;border-radius:10px;padding:10px 28px;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit">✕ إغلاق</button>`;
}
...
Object.assign(window, {
  ...
  switchTopicTab, playYT, openImagePage, openLightbox,
