...
function topicFull(d, key) {
  ...
  return `
    ...
    <div class="topic-hero fade-up" style="background:linear-gradient(135deg,var(--cat-700),var(--cat-500))">
      ...
      <p class="font-en" style="font-size:12px;color:var(--cat-100);opacity:.85;margin:0 0 14px">${d.categoryEn}</p>
      <div style="display:flex;gap:14px;padding-top:14px;border-top:1px solid rgba(255,255,255,.15)">
        <div style="display:flex;align-items:center;gap:4px">${ic('clock',13)}<span style="font-size:11px;color:white;font-weight:600">${d.duration}</span></div>
        <div style="width:1px;height:12px;background:rgba(255,255,255,.2)"></div>
        <div style="display:flex;align-items:center;gap:4px">${ic('bar-chart-3',13)}<span style="font-size:11px;color:white;font-weight:600">${d.difficulty}</span></div>
      </div>
    </div>
    ...
    <div style="display:flex;border-bottom:1px solid var(--border);background:var(--surface);position:sticky;top:0;z-index:10;direction:rtl">
      <button id="tab-btn-content" onclick="switchTopicTab('content','${key}')" style="flex:1;padding:12px 6px;font-size:12px;font-weight:700;border:none;background:none;cursor:pointer;border-bottom:2px solid var(--primary-500);color:var(--primary-600);display:flex;align-items:center;justify-content:center;gap:5px">
        ${ic('book-open',14)} المحتوى
      </button>
      <button id="tab-btn-videos" onclick="switchTopicTab('videos','${key}')" style="flex:1;padding:12px 6px;font-size:12px;font-weight:600;border:none;background:none;cursor:pointer;border-bottom:2px solid transparent;color:var(--text-muted);display:flex;align-items:center;justify-content:center;gap:5px">
        ${ic('clapperboard',14)} فيديوهات
      </button>
      <button id="tab-btn-images" onclick="switchTopicTab('images','${key}')" style="flex:1;padding:12px 6px;font-size:12px;font-weight:600;border:none;background:none;cursor:pointer;border-bottom:2px solid transparent;color:var(--text-muted);display:flex;align-items:center;justify-content:center;gap:5px">
        ${ic('image',14)} صور
      </button>
    </div>
    ...
    <div id="tab-videos" style="display:none">
      <div class="px-5" style="padding-top:24px;padding-bottom:40px;text-align:center">
        ...
        <p style="font-size:12px;color:var(--text-muted);line-height:1.7;margin:0">سيتم إضافة فيديوهات شرح لهذا الموضوع قريباً</p>
      </div>
    </div>
    ...
    <div id="tab-images" style="display:none">
      <div class="px-5" style="padding-top:24px;padding-bottom:40px;text-align:center">
        ...
        <p style="font-size:12px;color:var(--text-muted);line-height:1.7;margin:0">سيتم إضافة صور لهذا الموضوع قريباً</p>
      </div>
    </div>
  `;
}

function switchTopicTab(tab, key) {
  // إخفاء كل التابز
  ...
  if (activeBtn) {
    activeBtn.style.borderBottomColor = 'var(--primary-500)';
