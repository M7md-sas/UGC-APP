...
function topicFull(d, key) {
  ...
  const hasVideos = !!(m && m.videos && m.videos.length);
  const hasImages = !!(m && m.images && m.images.length);
  return `
    <div class="top-bar">
      <button class="icon-btn" onclick="goBack()">${ic('arrow-right')}</button>
    ...
    <div class="topic-tabs fade-up delay-1" id="tabs-${key}">
      ...
      <button class="topic-tab" id="tab-videos-${key}" onclick="switchTopicTab('videos','${key}',this)" style="${hasVideos?'':'display:none'}">${ic('play-circle',13)} فيديوهات</button>
      <button class="topic-tab" id="tab-images-${key}" onclick="switchTopicTab('images','${key}',this)" style="${hasImages?'':'display:none'}">${ic('image',13)} صور</button>
    </div>
