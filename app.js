  ...
  $('#page-home').innerHTML = `
    ...
    <div class="px-5 mb-5 fade-up delay-2">
      <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:10px">
        ${[
          ['tools','calculator','الحاسبات','rgba(192,57,43,.12)'],
          ['glossary','book-open','القاموس','rgba(52,152,219,.12)'],
          ['decision-trees','git-branch','شجرات القرار','rgba(39,174,96,.12)'],
          ['gap-analysis','target','الاختبار التشخيصي','rgba(107,91,149,.12)'],
          ['red-flags','alert-triangle','الإشارات الحمراء','rgba(230,126,34,.12)'],
          ['templates','file-text','قوالب التواصل','rgba(24,160,133,.12)'],
          ['emergency','siren','بروتوكولات الطوارئ','rgba(192,57,43,.14)'],
          ['library','library','المكتبة','rgba(30,58,95,.12)']
        ].map(([p,i,t,bg])=>`
          <button class="card card-interactive" onclick="goTo('${p}')" style="padding:16px 10px;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;gap:10px;min-height:90px">
            <p style="font-size:20px;font-weight:800;margin:0;line-height:1.2;color:var(--text)">${t}</p>
            <div style="width:28px;height:28px;border-radius:8px;background:${bg};display:flex;align-items:center;justify-content:center">${ic(i,14)}</div>
          </button>
        `).join('')}
      </div>
    </div>
